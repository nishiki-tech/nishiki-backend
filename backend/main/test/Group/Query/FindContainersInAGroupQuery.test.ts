...
import {FindContainersInAGroupQuery, InvalidUUId, GroupNotFound} from "src/Group/Query/FindContainersInAGroupQuery/FindContainersInAGroupQuery";
...
describe("Find containers in a group", () => {
    ...
    it("a group is not found", async () => {
        ...
        vi.spyOn(nishikiDynamoDb, "getGroup").mockReturnValueOnce(Promise.resolve(null)
        );
    ...
    it("no containers information has found", async () => {
        ...
        vi.spyOn(nishikiDynamoDb, "getGroup").mockReturnValueOnce(Promise.resolve({groupId: "", groupName: ""})
        );
        vi.spyOn(nishikiDynamoDb, "getContainer").mockReturnValueOnce(Promise.resolve(null));
        const result = await query.execute({groupId: GROUP_ID});
        console.log(result)
